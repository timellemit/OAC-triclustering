...
class Context:
    def __init__(self, input_file_adress):
        self.user_num, self.tag_num, self.res_num, \
        self.U, self.T, self.R, self.I = \
        parse_input.parse_input(input_file_adress)
    ...
    def triclusters2 (self, min_density = 0.5, density_check = True):
        Tdict, Tricl, PrOA, PrOC, PrAC = {}, [], {}, {}, {}
#         G = [i for i in xrange(len(self.U))]
#         M = [i for i in xrange(len(self.T))]
#         B = [i for i in xrange(len(self.R))]
        G, M, B = self.U, self.T, self.R
        for g in G:
            for m in M:
                PrOA[(g,m)] = self.r_prime(g, m)
        for g in G:
            for b in B:
                PrOC[(g,b)] = self.t_prime(g, b)
        for m in M:
            for b in B:
                PrAC[(m,b)] = self.u_prime(m, b)
        #i, len_I = 0.,len(I)
        for (g, m , b) in self.I:
            #writing current process into output file 
            #f.write(str(round(100*(i+1)/len_I),2) + '%\n')
            Tricl =  PrAC[(m,b)], PrOC[(g, b)], PrOA[(g, m)]
            Tkey = hash(str(Tricl))
            if density_check:
                dens = self.density(Tricl[0], Tricl[1], Tricl[2])
                if not Tdict.has_key(Tkey) and dens >= min_density:
...
if __name__ == '__main__':
    ...
    context = Context('input/context_gen_20x20x20.txt')
    min_density = 0.2
