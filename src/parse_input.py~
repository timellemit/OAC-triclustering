def parse_input(file_adr):
    context_txt = open(file_adr,'r')
    user_num, tags_num, res_num = [int(s) for s in \
                                   context_txt.readline().strip().split(',')]
    users = [int(u) for u in context_txt.readline().strip().split(',')]
    tags = [int(t) for t in context_txt.readline().strip().split(',')]
    resources = [int(r) for r in context_txt.readline().strip().split(',')]
